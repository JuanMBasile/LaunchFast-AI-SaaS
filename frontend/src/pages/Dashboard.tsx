  ...
  return (
    <div className="space-y-7">
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">{t('dashboard.overview')}</p>
      </div>
            ...
            <Card key={i}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold text-stone-900 tabular-nums ${stat.capitalize ? 'capitalize' : ''}`}>
                    {stat.value}
                  </p>
                </div>
              </div>
        ...
        <Card hover>
          <Link to="/dashboard/generator" className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-bold text-stone-900">{t('dashboard.generateProposal')}</h3>
              <p className="text-xs text-stone-400 mt-0.5">{t('dashboard.generateProposalDesc')}</p>
            </div>
        ...
        {user?.plan === 'free' && (
          <Card hover className="border-accent-200/60 bg-accent-50/30">
            <Link to="/pricing" className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-bold text-stone-900">{t('dashboard.upgradeToPro')}</h3>
                <p className="text-xs text-stone-400 mt-0.5">{t('dashboard.upgradeDesc')}</p>
              </div>
        ...
        {user?.plan === 'pro' && (
          <Card hover onClick={handleManageBilling}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-bold text-stone-900">{t('dashboard.manageBilling')}</h3>
                <p className="text-xs text-stone-400 mt-0.5">{t('dashboard.manageBillingDesc')}</p>
              </div>
              <ArrowRight className="h-4 w-4 text-stone-300" />
            </div>
          </Card>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-stone-900">{t('dashboard.recentProposals')}</h2>
          <Link
        ...
        {loading ? (
          <div className="text-center py-8 text-sm text-stone-400">{t('common.loading')}</div>
        ) : recentGenerations.length === 0 ? (
          <Card className="text-center py-10">
            <Wand2 className="h-10 w-10 text-stone-200 mx-auto mb-3" />
            <h3 className="text-sm font-medium text-stone-900 mb-1">{t('dashboard.noProposalsYet')}</h3>
            <p className="text-xs text-stone-400 mb-4">{t('dashboard.noProposalsDesc')}</p>
              ...
              <Link key={gen.id} to={`/dashboard/history/${gen.id}`}>
                <Card hover className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400 mt-0.5">{formatDate(gen.created_at, locale)}</p>
                  </div>
